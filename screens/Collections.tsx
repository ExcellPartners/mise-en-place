              ...
              {filteredRecipes.map(recipe => (
                <div key={recipe.id} onClick={() => onRecipeSelect(recipe)} className="flex flex-col gap-2 cursor-pointer group">
                  <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-[#1a1d14]">
                    <img src={formatImageUrl(recipe.imageUrl)} alt={recipe.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
        ...
        {activeSection === 'almostThere' && (
          <div className="px-4 pt-6">
            <div className="mb-6">
              ...
              <p className="text-[#b6baa1] text-sm font-medium leading-relaxed">
                Recipes you can make with just {pantry.length > 0 ? 'a few more ingredients' : 'ingredients from your pantry'}.
              </p>
            </div>
            {pantry.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center opacity-40 px-8">
                ...
                <p className="text-sm">Sync your Google Sheet to see ingredient-based recommendations.</p>
              </div>
                  ...
                  return (
                    <div key={`${entry.recipeId}-${idx}`} onClick={() => recipe && onRecipeSelect(recipe)}
                      ...
                      <div className="flex flex-col justify-center flex-1 min-w-0">
                        <p className="text-white font-bold text-base leading-tight line-clamp-1">{entry.recipeName || recipe?.title || 'Unknown Recipe'}</p>
