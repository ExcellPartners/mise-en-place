...
const Collections: React.FC<CollectionsProps> = ({
  ...
  const almostThereRecipes = useMemo(() => {
    const inStockNames = new Set(pantry.filter(p => (p.quantity ?? 0) > 0).map(p => p.name.toLowerCase().trim()));
    return recipes
      .map(recipe => {
  ...
  const sortedHistory = useMemo(() => {
    return [...cookedHistory].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 50);
  }, [cookedHistory]);
