...
interface PlannerProps {
  ...
  recipes: Recipe[];
  pantry: PantryItem[];
  pinnedIds: string[]; // Renamed from favoriteIds
  shoppingList?: RawShoppingEntry[];
  onScheduleMeal: (date: string, mealType: 'Breakfast' | 'Lunch' | 'Dinner' | 'Dessert' | 'Drink' | 'Snack', recipeId: string) => void;
...
const Toast: React.FC<{ message: string; isVisible: boolean; onClose: () => void }> = ({ message, isVisible, onClose }) => {
  ...
  return (
    <div 
      className={`fixed top-8 left-1/2 -translate-x-1/2 z-[300] transition-all duration-500 transform ${
...
const Planner: React.FC<PlannerProps> = ({ 
  ...
  // Use pinnedIds for the picker list
  const pinnedRecipes = useMemo(() => {
    return recipes.filter(r => pinnedIds.includes(r.id));
  }, [recipes, pinnedIds]);

  const getMealPlans = (type: string, date: string) => {
    return mealPlans.filter(p => p.date === date && p.mealType === type);
  };

  const getRecipe = (plan: MealPlan | undefined) => {
    return plan ? recipes.find(r => r.id === plan.recipeId) : null;
  };

  const showThemedToast = (msg: string) => {
  ...
  const handleShareItinerary = async () => {
    ...
    let text = "My Kitchen Companion Itinerary\n\n";
  ...
  const renderSlot = (type: 'Breakfast' | 'Lunch' | 'Dinner' | 'Dessert' | 'Drink', icon: string) => {
    const plans = getMealPlans(type, selectedDate);
    
    return (
      <div 
        className={`flex flex-col bg-[#2a2c21] rounded-3xl p-5 border border-white/5 transition-all relative overflow-hidden ${
          plans.length > 0 ? 'shadow-2xl' : 'border-2 border-dashed border-[#3b3e2e]'
        }`}
      >
                ...
                <div key={index} className="flex flex-col gap-4 border-b border-white/5 last:border-0 pb-4 last:pb-0 relative">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 overflow-hidden">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[#636b2f] text-[10px] font-black uppercase tracking-widest">{recipe.prepTime + recipe.cookTime} MINS</span>
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex-1 h-12 rounded-xl bg-[#3b3e2e] text-white flex items-center justify-center gap-2 transition-all active:scale-95"
                    >
                      <span className="material-symbols-outlined text-lg pl-3">shopping_basket</span>
                      <span className="text-xs font-black uppercase tracking-widest">Ingredients</span>
                    </button>
                    <button 
                      onClick={() => onStartCooking?.(recipe)}
                      className="flex-1 h-12 rounded-xl bg-[#636b2f] text-white flex items-center justify-center gap-2 transition-all active:scale-95"
                    >
                      <span className="material-symbols-outlined text-lg fill-1">skillet</span>
                      <span className="text-xs font-black uppercase tracking-widest">Cook Now</span>
                    </button>
                  </div>
                </div>
              );
    ...
    <div className="flex h-screen w-full flex-col bg-[#1c1d15] text-white font-display max-w-[480px] mx-auto relative overflow-hidden">
      ...
      <header className="flex flex-col z-10 bg-[#1c1d15]/95 backdrop-blur-md sticky top-0 border-b border-white/5 pb-2 header-safe-pt">
        <div className="flex items-center px-4 pb-2 justify-between">
          <button 
            onClick={onBack}
            className="flex size-10 shrink-0 items-center justify-center text-white active:scale-90 transition-transform"
          >
          ...
          <div className="flex w-10 items-center justify-end">
            <button 
              onClick={() => {
                ...
                setViewMonth(new Date(selectedDate.replace(/-/g, '/')));
        ...
        <div className="flex justify-between gap-1 px-4 py-4 overflow-x-auto no-scrollbar">
          {weekDays.map(day => (
            <button
              ...
              className={`flex h-16 w-[12.5%] min-width-[54px] shrink-0 flex-col items-center justify-center gap-1 rounded-2xl transition-all ${
      ...
      <main className="flex-1 overflow-y-auto px-4 pt-6 pb-64 no-scrollbar scroll-smooth">
        ...
        <div className="space-y-6">
          {renderSlot('Breakfast', 'coffee')}
          {renderSlot('Lunch', 'sunny')}
          {renderSlot('Dinner', 'dark_mode')}
          {renderSlot('Dessert', 'icecream')}
          {renderSlot('Drink', 'local_bar')}
        </div>
      </main>

      <div className="fixed bottom-[84px] left-1/2 -translate-x-1/2 w-full max-w-[480px] z-40">
      ...
      {isCalendarOpen && (
        <div className="fixed inset-0 z-[200] bg-[#0f110c] flex flex-col animate-in slide-in-from-bottom duration-300 max-w-[480px] mx-auto">
          <header className="pt-4 px-4 header-safe-pt">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  className="text-sm font-semibold text-[#636b2f]"
                >
                  Today
                </button>
                <button onClick={() => setTempDate('')} className="text-sm font-semibold text-muted">Clear</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-7 text-center text-[10px] font-bold uppercase tracking-widest text-muted/60 mb-2">
          ...
          <main className="flex-1 overflow-y-auto px-4 no-scrollbar">
            <div className="grid grid-cols-7 gap-y-1 relative">
              {calendarDays.map((d, i) => {
                ...
                return (
                  <button key={i} onClick={() => setTempDate(iso)} className={`h-12 flex items-center justify-center text-sm font-bold transition-all relative z-10 ${!d.current ? 'text-muted/20' : 'text-white'}`}>
                    {isSelected ? <span className="bg-[#636b2f] size-9 flex items-center justify-center rounded-full text-white shadow-lg shadow-[#636b2f]/20 scale-110">{d.day}</span> : <span>{d.day}</span>}
                  </button>
                );
              })}
            </div>
            <div className="mt-12 mb-8">
              <button disabled={!tempDate} onClick={() => { if (tempDate) { setSelectedDate(tempDate); setIsCalendarOpen(false); } }} className="w-full bg-[#636b2f] py-4 rounded-2xl font-black text-lg shadow-xl shadow-[#636b2f]/10 active:scale-[0.98] transition-all disabled:opacity-30">Apply Selection</button>
            </div>
          </main>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[250] bg-[#1c1d15] flex flex-col animate-in slide-in-from-bottom duration-300 max-w-[480px] mx-auto">
          ...
          <main className="flex-1 overflow-y-auto px-4 py-6 no-scrollbar">
            {Object.keys(groupedSummary).length > 0 ? (
              (Object.entries(groupedSummary) as [string, MealPlan[]][]).map(([date, plans]) => (
                <div key={date} className="mb-10">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="h-0.5 flex-1 bg-[#636b2f]/20"></div>
                    <p className="text-[#636b2f] text-[10px] font-black uppercase tracking-[0.3em]">{new Date(date.replace(/-/g, '/')).toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}</p>
                    <div className="h-0.5 flex-1 bg-[#636b2f]/20"></div>
                  </div>
                        ...
                        <div key={`${plan.mealType}-${idx}`} className="flex gap-4 p-4 bg-white/5 rounded-2xl border border-white/5">
                          <div className="size-12 rounded-xl bg-[#636b2f]/10 flex items-center justify-center text-[#636b2f] shrink-0 border border-[#636b2f]/10">
                            <span className="material-symbols-outlined">
                              {plan.mealType === 'Breakfast' ? 'coffee' : 
                               plan.mealType === 'Lunch' ? 'sunny' : 
                               plan.mealType === 'Dinner' ? 'dark_mode' : 
                               plan.mealType === 'Dessert' ? 'icecream' : 'local_bar'}
                            </span>
                          </div>
                          <div className="flex flex-col justify-center">
                            <p className="text-[#b6baa1] text-[10px] font-black uppercase tracking-widest leading-none mb-1">{plan.mealType}</p>
                            <h4 className="text-white font-bold text-lg leading-tight">{recipe?.title || 'Unknown Recipe'}</h4>
                          </div>
                        </div>
                      );
                    })}
              ...
              <div className="h-full flex flex-col items-center justify-center text-center opacity-40 px-10">
                ...
                <p className="text-sm mt-2">Go back and pin some recipes to build your weekly itinerary.</p>
              </div>
            )}
          </main>
          <footer className="p-6 border-t border-white/5 bg-[#1c1d15] space-y-3">
            <button 
              ...
              className="w-full bg-white/5 border border-white/10 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 active:bg-white/10 transition-all shadow-xl"
            >
      ...
      {isPickerOpen && (
        <div className="fixed inset-0 z-[150] flex items-end justify-center">
          ...
          <div className="relative w-full max-w-[480px] bg-[#2a2c21] rounded-t-[3rem] p-6 pb-12 flex flex-col gap-6 animate-in slide-in-from-bottom duration-300">
            ...
            <div className="grid grid-cols-2 gap-4 max-h-[50vh] overflow-y-auto pr-2 no-scrollbar">
              {pinnedRecipes.length > 0 ? (
                pinnedRecipes.map(recipe => (
                  <button 
                    key={recipe.id} 
                    onClick={() => { onScheduleMeal(isPickerOpen.date, isPickerOpen.type as any, recipe.id); setIsPickerOpen(null); }} 
            ...
            <button onClick={() => setIsPickerOpen(null)} className="w-full h-14 rounded-2xl bg-white/5 text-white font-black uppercase tracking-widest active:bg-white/10">Cancel</button>
          </div>
        </div>
      )}
      ...
      {selectorData && (
        <div className="fixed inset-0 z-[250] flex items-end">
          ...
          <div className="relative w-full">
            {/* Servings adjuster bar above the selector */}
            <div className="bg-[#2a2c21] border-t border-white/10 px-6 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center bg-[#1c1d15] rounded-xl p-1 border border-white/5">
                <button onClick={() => setSelectorData(prev => prev ? { ...prev, servings: Math.max(1, prev.servings - 1) } : null)}
                  className="size-8 flex items-center justify-center text-[#636b2f] active:scale-90">
                  <span className="material-symbols-outlined text-xl">remove</span>
                </button>
                <span className="px-3 font-black text-white min-w-[40px] text-center text-sm">{selectorData.servings}x</span>
