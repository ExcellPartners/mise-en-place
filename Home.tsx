...
const Home: React.FC<HomeProps> = ({
  recipes = [], pinnedIds = [], likedIds = [], mealPlans = [],
  onTogglePin, onRecipeSelect, onSettingsOpen, onPlannerOpen, onCollectionsOpen,
    ...
    <div className="w-full min-h-screen bg-[#000000]">
      <header className="flex items-center justify-between mb-2 px-4 header-safe-pt pt-6">
        <div className="flex items-center gap-3">
          <div className="size-14 rounded-2xl overflow-hidden shadow-inner border border-white/10 p-1">
            <img src="https://res.cloudinary.com/dwf0blscr/image/upload/v1769963656/Mise_en_Place_Icon_-_Green_4x_uv2xxn.png" alt="Mise en Place" className="w-full h-full object-contain" />
          </div>
        ...
        <button onClick={onSettingsOpen} className="w-11 h-11 flex items-center justify-center rounded-full bg-[#1a1d14] border border-gray-800 text-gray-400 active:scale-90 transition-transform shadow-xl">
          <span className="material-symbols-outlined text-xl">settings</span>
        </button>
      </header>

      <div className="mt-6 mb-6 px-4">
        <button onClick={onCollectionsOpen} className="w-full relative overflow-hidden rounded-3xl h-[160px] group shadow-2xl border border-white/5">
          ...
          <div className="relative h-full flex flex-col justify-center px-6 items-start text-left">
            ...
            <p className="text-[#b6baa1] text-xs font-medium max-w-[200px] leading-relaxed">Explore curated suites, lifestyle labs, and global galleries.</p>
      ...
      <div className="mb-4 px-4">
        <div className="relative flex items-center group">
          ...
          <input type="text" value={searchQuery} onChange={(e) => handleSearchChange(e.target.value)} placeholder="Search your collection..."
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar py-3 -mx-4 px-4">
          {categories.map(cat => (
            <button key={cat} onClick={() => handleCategoryChange(cat)}
              className={`flex h-9 shrink-0 items-center justify-center px-4 rounded-xl transition-all duration-300 hover:scale-105 active:scale-95 ${activeCategory === cat ? 'bg-[#636b2f] text-white shadow-lg shadow-[#636b2f]/20 font-bold scale-105 border border-[#636b2f]' : 'bg-[#1c1d15] border border-gray-800 text-gray-400 font-bold'}`}>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-5 mb-3 gap-2">
          <p className="text-white/30 text-[10px] font-black uppercase tracking-widest whitespace-nowrap shrink-0">{displayRecipes.length} recipes • {safePage}/{totalPages}</p>
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar">
            <button onClick={() => goToPage(Math.max(1, safePage - 1))} disabled={safePage === 1} className="size-7 flex items-center justify-center rounded-lg bg-white/5 text-white/40 disabled:opacity-20 active:scale-90 shrink-0">
            ...
            <div className="flex items-center gap-1 max-w-[200px] overflow-x-auto no-scrollbar">
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((item, idx) => item === 'gap'
                  ...
                  : <button key={item} onClick={() => goToPage(item as number)} className={`size-7 flex items-center justify-center rounded-lg text-[10px] font-black shrink-0 ${item === safePage ? 'bg-[#636b2f] text-white' : 'bg-white/5 text-white/40'}`}>{item}</button>
                )}
            </div>
            <button onClick={() => goToPage(Math.min(totalPages, safePage + 1))} disabled={safePage === totalPages} className="size-7 flex items-center justify-center rounded-lg bg-white/5 text-white/40 disabled:opacity-20 active:scale-90 shrink-0">
          ...
          return (
            <div key={recipe.id} className="flex flex-col gap-3 group cursor-pointer" onClick={() => onRecipeSelect(recipe)}>
                ...
                {finalImageUrl ? (
                  <img src={finalImageUrl} alt={recipe.title} referrerPolicy="no-referrer"
                    ...
                    onError={(e) => {
                      ...
                      if (img.src.includes('uc?export=view')) { const m = img.src.match(/id=([a-zA-Z0-9_-]+)/); if (m) { img.src = `https://drive.google.com/thumbnail?id=${m[1]}&sz=w1000`; return; } }
              ...
              <div className="px-1">
                ...
                <p className="text-[#b6baa1] text-[10px] font-black uppercase tracking-widest mt-1">{(recipe.prepTime || 0) + (recipe.cookTime || 0)} MIN • {recipe.difficulty || 'Easy'}</p>
              </div>
            </div>
          );
        })}
